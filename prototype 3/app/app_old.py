...
class IngredientPredictor(nn.Module):
    def __init__(self, num_ingredients, num_classes=52, backbone='efficientnet_b0'):
        ...
        self.classifier_class = nn.Sequential(
            nn.Linear(feature_dim, 512), nn.ReLU(), nn.Dropout(0.5), nn.Linear(512, num_classes)
        )
        self.classifier_ingredients = nn.Sequential(
            nn.Linear(feature_dim + num_classes, 512), nn.ReLU(), nn.Dropout(0.5),
            nn.Linear(512, num_ingredients), nn.Sigmoid()
        )
...
def find_best_food_match(title, food_map):
    ...
    for food_name in food_map.keys():
        ...
        # Check if food name is in title or title is in food name
...
def enhance_ingredients_with_food_map(predicted_ingredients, title, food_map):
    ...
    if matched_food:
        # Get required ingredients for this food
        ...
        # Normalize predicted ingredients for comparison
        ...
        for req_ing in required_ingredients:
            # Check if this required ingredient is already in predicted ingredients
            found = False
...
def predict_ingredients(image_path, model_path, vocab_path, food_map_path, threshold=0.7, device=device):
    ...
    image_tensor = load_image_for_ingredients(image_path).to(device)
...
def generate_title(image_path, blip_processor_path, blip_model_path, device=device):
    ...
    generated_ids = model.generate(
        **inputs, max_length=20, num_beams=5, length_penalty=1.0, early_stopping=True
    )
...
def generate_recipe_instructions(image_path, predicted_ingredients, title, t5_model_path, device=device):
    ...
    with torch.no_grad():
        t5_outputs = t5_model.generate(
            input_ids=t5_inputs['input_ids'], attention_mask=t5_inputs['attention_mask'],
            max_length=256, num_beams=5, length_penalty=1.2, early_stopping=True, no_repeat_ngram_size=2
        )
...
st.write("Upload a food image. Get back predicted ingredients (enhanced with food mapping), a dish title, and full cooking recipe.")

with st.sidebar:
    st.header("Model File Paths")
    INGREDIENT_MODEL_PATH = st.text_input("Ingredient model (.pth):", "best_ingredient_model_new.pth")
    VOCAB_PATH = st.text_input("Vocabulary (.json):", "ingredient_vocabulary_new.json")
    FOOD_MAP_PATH = st.text_input("Food mapping (.json):", "food_map.json")
    BLIP_PROCESSOR_PATH = st.text_input("BLIP processor (not used, put any):", "blip_processor")
    BLIP_MODEL_PATH = st.text_input("BLIP title model (.pth):", "blip_finetuned_title_gen.pth")
    T5_MODEL_PATH = st.text_input("T5 recipe model (.pth):", "t5_base_recipe_instr.pth")
...
if uploaded_file is not None:
    ...
    if st.button("Predict and Generate Recipe"):
        with st.spinner("Processing..."):
            ...
            # Fix specific case: rename "Pizza dough" to "Pizza"
            if title and title.lower().strip() == "pizza dough":
                title = "Pizza"
            
            st.markdown("**Generated Dish Title:**")
            ...
            if not predicted_ingredients:
                ...
                # Step 3: Load food map and enhance ingredients (backend processing)
                ...
                if food_map:
                    ...
                    # Use enhanced ingredients for final output
                    final_ingredients = enhanced_ingredients
                else:
                    # Use original predicted ingredients if food map not available
                    final_ingredients = predicted_ingredients
                
                # Show only the final enhanced ingredients
                st.markdown("**Predicted Ingredients:**")
                st.write(", ".join(final_ingredients))

                # Step 4: Generate recipe with enhanced ingredients
